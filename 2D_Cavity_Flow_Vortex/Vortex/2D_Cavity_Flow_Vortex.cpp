...
void Cavity_flow(float u[nx][ny],float v[nx][ny],float p[nx][ny],float u_vortex[nx][ny],float v_vortex[nx][ny],float rho, float nu, float dt, float dx, float dy)
{
    FILE *fp; FILE *fpv; FILE *fpu; FILE *fpp;
    fp = fopen("2D_Cavity_Flow_Vortex_final_Solution.dat","w");
...
// Boundary Conditions
        ...
        for(i=0;i<nx;i++)
            ...
            for (i=0 ; i < nx ; i++)
            {
                for (j=0 ; j < ny ; j++)
                {
                    fprintf(fp,"%f\t %f\t %f\t %f\t %f\n",x[i],y[j],u[i][j] + u_vortex[i][j],v[i][j] + v_vortex[i][j],p[i][j]);
                }
            }
            fprintf(fp,"\n\n");
...
int main()
{
    ...
    float dt = 0.001;
    float gamma = 0.5;
    float x_vortex , y_vortex;
    int x_position,y_position;
    float u[nx][ny], v[nx][ny], p[nx][ny];
    float u_vortex[nx][ny] , v_vortex[nx][ny];
    float x[nx],y[ny];
    ...
    x_position = (nx-1)/2  ;
    y_position = (ny-1)/2  ;
    x_vortex = x[x_position];
    y_vortex = y[y_position];
    ...
    for (i = 0; i<nx ; i++)
    {
        for(j=0; j<ny ; j++)
        {
            ...
            {
                u_vortex[i][j] = +gamma / (2 * M_PI) * ((y[j] - y_vortex) / (pow((x[i] - x_vortex),2) + pow((y[j] - y_vortex),2)));
                v_vortex[i][j] = (-1)*gamma / (2 * M_PI) * ((x[i] - x_vortex) / (pow((x[i] - x_vortex),2) + pow((y[j] - y_vortex),2)));
            }
...
//Initializing values of U, V and P
   ...
   Cavity_flow(u,v,p,u_vortex,v_vortex,rho,nu,dt,dx,dy);
}
